...
void heap_printInfo(bool sizes, bool types, bool freed, bool chain) {
  ...
  log_printf("RAM allocated: "N64u"\n", total);
  log_printf("heap in use: "N64u"\n", used);
  #if MM!=0
    if (sizes) {
      for (i32 i = 2; i < 64; i++) {
        for (i32 j = 0; j < MM; j++) {
          ...
          if (count>0) log_printf("size %llu: "N64u"\n", (1ULL<<o)*(j?3:1), count);
        }
      }
    }
    if (types) {
      ...
      for (i32 i = 0; i < t_COUNT; i++) {
        ...
        if (count>0) log_printf("type %d/%s: count "N64u", total size "N64u"\n", i, type_repr(i), count, heap_PISizes[i]);
      }
    }
    if (freed || chain) {
      if (freed) {
      ...
      for (i32 i = 2; i < 64; i++) {
        for (i32 j = 0; j < MM; j++) {
          ...
          if (cf!=0 || cc!=0) {
            if (chain && freed) log_printf("freed size %llu: count "N64u", chain "N64u"\n", (1ULL<<o)*(j?3:1), cf, cc);
            else log_printf("freed size %llu: "N64u"\n", (1ULL<<o)*(j?3:1), freed? cf : cc);
          }
        }
      }
    }
