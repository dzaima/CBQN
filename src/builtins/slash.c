...
B slash_im(B t, B x) {
  ...
  #undef CASE
    ...
    case el_f64: {
      ...
      for (i = 0; i < xia; i++) { f64 c=xp[i]; if (!q_fuszo(&cu, c)) slash_im_bad(c); if (c<=max) break; max=c; }
      for (j = i; j < xia; j++) { f64 c=xp[j]; if (!q_fuszo(&cu, c)) slash_im_bad(c); max=c>max?c:max; }
